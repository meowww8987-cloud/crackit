      ...
      <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
        ...
        <div className="sticky top-0 z-10 px-5 pt-4 pb-3 glass rounded-t-3xl" style={{ borderBottom: '1px solid var(--border)' }}>
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={onClose} className="w-8 h-8 rounded-lg bg-foreground/5 flex items-center justify-center text-muted-foreground"><X size={16} /></button>
          </div>
        </div>
        <div className="overflow-y-auto scroll-area px-5 py-4">
          {showAdd ? (
            <div className="glass rounded-xl p-3 mb-4 space-y-3">
              <div>
                <label className="text-[10px] font-semibold text-muted-foreground block mb-1.5">SUBJECT</label>
                <div className="flex gap-1 flex-wrap">
                  {SUBJECTS.map((s) => (
                    <button key={s} onClick={() => { setSubject(s); setChapter(''); vibrate(6); }}
                      className={cn('px-2 py-1 rounded-md text-[10px] font-semibold', subject === s ? 'text-black' : 'bg-foreground/5 text-muted-foreground')}
              ...
              {availableChapters.length > 0 && (
                <div>
                  <label className="text-[10px] font-semibold text-muted-foreground block mb-1.5">CHAPTER</label>
                  <select value={chapter} onChange={(e) => setChapter(e.target.value)}
                    className="w-full bg-foreground/5 border border-border rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:border-teal-400/50">
              ...
              <div>
                <label className="text-[10px] font-semibold text-muted-foreground block mb-1.5">QUESTION *</label>
                <textarea value={question} onChange={(e) => setQuestion(e.target.value)} placeholder="Type your doubt here..."
                  className="w-full bg-foreground/5 border border-border rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:border-teal-400/50 min-h-[60px]" autoFocus />
              </div>
              <div>
                <label className="text-[10px] font-semibold text-muted-foreground block mb-1.5">SOURCE</label>
                <div className="flex gap-1">
                  {(['self', 'class', 'mock'] as const).map((src) => (
                    <button key={src} onClick={() => { setSource(src); vibrate(6); }}
                      className={cn('px-2 py-1 rounded-md text-[10px] font-medium capitalize', source === src ? 'bg-teal-500 text-black' : 'bg-foreground/5 text-muted-foreground')}>
              ...
              <button onClick={handleAdd} disabled={!question.trim()}
                className={cn('w-full py-2.5 rounded-lg font-bold text-xs transition', question.trim() ? 'bg-gradient-to-r from-teal-500 to-green-500 text-black' : 'bg-foreground/5 text-muted-foreground/60')}>
                <Plus size={12} className="inline mr-1" /> Add Doubt
              </button>
            </div>
          ) : resolvingId ? (
            <div className="glass rounded-xl p-3 mb-4 space-y-3">
              <label className="text-[10px] font-semibold text-muted-foreground block mb-1.5">SOLUTION *</label>
              <textarea value={solution} onChange={(e) => setSolution(e.target.value)} placeholder="How was this doubt solved?"
                className="w-full bg-foreground/5 border border-border rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:border-teal-400/50 min-h-[80px]" autoFocus />
              <div className="flex gap-2">
                <button onClick={() => { setResolvingId(null); setSolution(''); }} className="flex-1 py-2 rounded-lg bg-foreground/5 text-muted-foreground text-xs font-semibold">Cancel</button>
                <button onClick={handleResolve} disabled={!solution.trim()} className={cn('flex-1 py-2 rounded-lg font-bold text-xs', solution.trim() ? 'bg-green-500 text-black' : 'bg-foreground/5 text-muted-foreground/60')}>Resolve</button>
              </div>
            </div>
          ) : null}
          ...
          {pending.length > 0 && (
            <div className="mb-4">
              <h3 className="text-[10px] font-bold uppercase text-muted-foreground mb-2">📌 Pending ({pending.length})</h3>
                  ...
                  return (
                    <div key={d.id} className="glass rounded-xl p-2.5" style={{ borderLeft: `2px solid ${c.hex}` }}>
                      <div className="flex items-center gap-1.5 mb-1">
                        ...
                        {d.chapter && <span className="text-[9px] text-muted-foreground">{d.chapter}</span>}
                        <span className="text-[9px] text-muted-foreground/60 ml-auto">{new Date(d.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                      </div>
                      <p className="text-xs text-foreground/85 mb-2">{d.question}</p>
          ...
          {resolved.length > 0 && (
            <div>
              <h3 className="text-[10px] font-bold uppercase text-muted-foreground mb-2">✅ Resolved ({resolved.length})</h3>
              <div className="space-y-2">
                {resolved.slice(0, 10).map((d) => {
                  ...
                  return (
                    <div key={d.id} className="glass rounded-xl p-2.5 opacity-70">
                      ...
                      <p className="text-xs text-muted-foreground mb-1 line-through">{d.question}</p>
          ...
          {doubts.length === 0 && !showAdd && (
            <div className="text-center py-8">
              <HelpCircle size={32} className="text-foreground/10 mx-auto mb-2" />
              <p className="text-xs text-muted-foreground">No doubts yet. Add any question you're stuck on!</p>
            </div>
          )}
        </div>
      </motion.div>
