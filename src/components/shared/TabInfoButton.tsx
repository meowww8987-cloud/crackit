...
interface Props {
  /** Which tab this button belongs to. Determines the info content. */
  tab: 'home' | 'study' | 'syllabus' | 'history' | 'tests' | 'stats';
}
...
const TAB_INFO: Record<Props['tab'], { title: string; subtitle: string; sections: InfoSection[] }> = {
  home: {
    title: 'Home Tab',
  ...
  study: {
    ...
    sections: [
      ...
      },
      {
        title: 'Hidden Features',
        items: [
          { icon: ' grip', text: 'Long-press the Study tab (bottom nav) to open Quick Actions: Free Study + Daily Recall Challenge.' },
          ...
          { icon: '🔄', text: 'Full-screen focus session auto-rotates with your phone (all 4 directions). Tap the rotate button to cycle manually.' },
        ],
      },
    ],
  },
  syllabus: {
    ...
    sections: [
      ...
      },
      {
        title: 'Hidden Features',
        items: [
          { icon: ' grip', text: 'Long-press the Syllabus tab (bottom nav) to open: Build Syllabus + Formula Vault.' },
...
/**
 * TabInfoButton — a small tutorial/info icon (HelpCircle) that floats in
 * the bottom-right corner of each tab (except Settings). When tapped, it
 * opens a bottom sheet showing:
 *   - What the tab does
 *   - Quick actions (how to use it)
 *   - Hidden features (things you might not discover on your own)
 *
 * The icon sits just above the bottom nav (bottom-24) so it doesn't overlap.
 */
export function TabInfoButton({ tab }: Props) {
