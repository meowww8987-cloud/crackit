...
// Modern resource config — using lucide icons instead of emoji for consistency
...
export function LectureResourceRow({ lecture, chapter, subject, index, onEdit }: Props) {
  ...
  return (
    ...
    >
      {/* === Header row: +Today + L# + topic + progress === */}
      <div className="flex items-center gap-2 mb-2.5">
        ...
        <span className={cn(
          ...
          isComplete ? 'text-t-muted line-through' : 'text-t-primary'
        )}>
          {lecture.topic}
        </span>
        ...
        {/* Custom badge */}
        {lecture.isCustom && (
          <span className="text-[8px] font-bold px-1.5 py-0.5 rounded bg-purple-500/15 text-purple-500 dark:text-purple-400 border border-purple-500/20">
            CUSTOM
          </span>
        )}
        ...
        <span
          ...
          style={{
            color: isComplete ? '#22c55e' : doneCount > 0 ? color.hex : 'var(--text-muted)',
      ...
      {/* === Resource toggle buttons — modern, clear states === */}
        ...
        {RESOURCES.map((res) => {
          ...
          return (
            <button
              ...
              className={cn(
                'py-2 rounded-lg flex flex-col items-center justify-center gap-0.5 transition border',
                isDone
                  ? 'text-black border-transparent'
                  : 'border-white/10 bg-white/[0.03] text-t-muted hover:bg-white/[0.07] hover:border-white/20'
              )}
              style={isDone ? { background: res.doneColor } : undefined}
              title={res.label}
            >
              <Icon size={13} fill={isDone ? 'currentColor' : 'none'} />
              <span className="text-[8px] font-semibold uppercase tracking-wide">{res.label}</span>
              {isDone && <Check size={9} strokeWidth={3} className="absolute" style={{ marginTop: '-20px', marginRight: '-20px' }} />}
            </button>
          );
        })}
      </div>
        ...
        <div className="flex items-center gap-3 mt-2.5 text-[10px] tabular text-t-muted flex-wrap">
          {lecture.timeSpentSec && lecture.timeSpentSec > 0 && (
            <span className="flex items-center gap-1">
              <Play size={9} className="text-green-500 dark:text-green-400" fill="currentColor" />
              <span className="text-t-secondary font-medium">{formatHM(lecture.timeSpentSec)}</span>
            </span>
          )}
            ...
            >
                ...
                <div className="flex gap-1 flex-wrap">
                  {ACTIVITIES.map((a) => (
                    <button
                      ...
                      className={cn(
                        'px-2.5 py-1 rounded-full text-[10px] font-medium transition border',
                        activity === a
                          ? 'bg-teal-500 text-black border-teal-500'
                          : 'bg-white/5 text-t-secondary border-white/10 hover:bg-white/10'
                      )}
                    >
                      {a}
                    </button>
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="text-[9px] font-bold text-t-muted uppercase tracking-wide">Expected Time</label>
                  <span className="text-[11px] font-bold tabular text-teal-500 dark:text-teal-400">{expectedMinutes} min</span>
                </div>
                <input
                  type="range"
                  min={15}
              ...
              <button
                ...
                className="w-full py-2.5 rounded-lg text-[11px] font-bold text-black transition active:scale-95 flex items-center justify-center gap-1.5"
                style={{ background: color.hex }}
              >
                <Check size={11} /> Add to Today ({expectedMinutes}m)
              </button>
            </div>
          </motion.div>
        )}
          ...
          >
            <div className="flex gap-1.5 mt-2.5 pt-2.5 border-t border-white/10">
              <button
                ...
                className="flex-1 py-1.5 rounded-lg bg-white/5 text-t-secondary text-[10px] font-semibold flex items-center justify-center gap-1 border border-white/10 hover:bg-white/10"
              >
                <Pencil size={10} /> Edit
              </button>
              <button
                ...
                className="flex-1 py-1.5 rounded-lg bg-red-500/10 text-red-500 dark:text-red-400 text-[10px] font-semibold flex items-center justify-center gap-1 border border-red-500/20 hover:bg-red-500/20"
              >
